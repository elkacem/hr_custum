...
//        dd($supportedCurrencies, $parameters);
...
//        dd($gateway);
        ...
        $gateway->mode = $request->input('mode');
        $gateway->save();
        ...
        if ($request->has('currency')) {
            ...
            foreach ($request->currency as $key => $currency) {
                ...
                $gatewayCurrency->full_payment_discount = $currency['full_payment_discount'] ?? 0; // New field for full payment discount
                $gatewayCurrency->allow_one_day_pay = $currency['allow_one_day_pay'] ?? 0; // New field for one day pay
...
//        dd($supportedCurrencies);
        ...
        if ($request->has('currency')) {
            foreach ($request->currency as $key => $currency) {
                ...
                $validationRule['currency.' . $key . '.full_payment_discount'] = 'nullable|numeric|gte:0|max:100'; // New validation for full payment discount
                $validationRule['currency.' . $key . '.allow_one_day_pay'] = 'nullable|numeric|gte:0|max:100'; // New validation for one day pay
...
//                $supportedCurrencies = collect(removeElement($supportedCurrencies, $currency['currency']))->implode(',');
                $supportedCurrencies = collect($supportedCurrencies)->implode(',');
                ...
                $customAttributes['currency.' . $key . '.full_payment_discount'] = $currencyIdentifier . ' ' . keyToTitle('full_payment_discount'); // New attribute for full payment discount
                $customAttributes['currency.' . $key . '.allow_one_day_pay'] = $currencyIdentifier . ' ' . keyToTitle('allow_one_day_pay'); // New attribute for one day pay
