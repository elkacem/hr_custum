...
//        $section = config("sections.$key"); // or a custom static array
//        dd($section);
        abort_if(!$section || !$section->builder, 404);

        $content = Frontend::where('data_keys', $key . '.content')->orderBy('id', 'desc')->first();
        $elements = Frontend::where('data_keys', $key . '.element')->orderBy('id', 'desc')->get();
...
//        }
    ...
    public function frontendElementSlugCheck($key,$id = null){
        $content = Frontend::where('data_keys', $key . '.element')->where('slug',request()->slug);
        if ($id) {
