...
class Deposit extends Model
{
    ...
    {
        return $this->belongsTo(PlanLog::class);
    }
    ...
    {
        return $query->where('method_code', '<', 5000)->where('status', Status::PAYMENT_SUCCESS);
    }
    ...
    public function scopeOverlapping(Builder $q, Carbon $start, Carbon $end, string $startCol = 'pick_time', string $endCol = 'drop_time'): Builder
    {
        // Overlap <=> start < $end AND end > $start
        return $q->where($startCol, '<', $end)
            ->where($endCol, '>', $start);
    }
